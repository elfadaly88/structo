...
var AuthService = class _AuthService {
  ...
  googleLogin(idToken, subscriptionPlan) {
    const googleApiUrl = `${environment.apiUrl}/google-auth/google-login`;
    return this.http.post(googleApiUrl, { idToken, subscriptionPlan }).pipe(tap((response) => {
      if (response.success && response.data) {
        this.setSession(response.data);
      }
    }));
  ...
  initializeNotifications(user) {
    setTimeout(() => {
      try {
        ...
        notificationService.initializeOneSignal(user.userId || user.id, user.email);
      } catch (err) {
  ...
  getEmailFromToken(token) {
    try {
      const payload = token.split(".")[1];
      const decoded = JSON.parse(atob(payload));
      return decoded["unique_name"] || decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"] || decoded["email"] || "";
    } catch (e) {
      return "";
    }
  }
  getNameFromToken(token) {
    try {
      const payload = token.split(".")[1];
      const decoded = JSON.parse(atob(payload));
      return decoded["name"] || decoded["unique_name"] || decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname"] || "User";
    } catch (e) {
      return "User";
    }
  }
  setSession(authData) {
    const _a = authData, { token, refreshToken } = _a, userData = __objRest(_a, ["token", "refreshToken"]);
    const email = this.getEmailFromToken(token);
    const name = userData.name || this.getNameFromToken(token);
    const session = __spreadProps(__spreadValues({}, userData), { email, name });
  ...
  updateProfileCompletionStatus(isComplete) {
    ...
    if (user) {
      const updatedUser = __spreadProps(__spreadValues({}, user), { isProfileComplete: isComplete });
...
var NotificationService = class _NotificationService {
  ...
  hubConnection = null;
  static oneSignalInitialized = false;
  static oneSignalInitInFlight = false;
  static oneSignalSyncedUserId = null;
  initializeOneSignal(userId, email) {
    if (!userId) {
      return;
    }
    if (_NotificationService.oneSignalSyncedUserId === userId && _NotificationService.oneSignalInitialized) {
      return;
    }
    if (_NotificationService.oneSignalInitInFlight) {
      return;
    }
    _NotificationService.oneSignalInitInFlight = true;
    ...
    window.OneSignalDeferred.push(async (OneSignal) => {
      const syncIdentity = async () => {
        try {
          ...
          _NotificationService.oneSignalSyncedUserId = userId;
        } catch (e) {
          console.warn("[OneSignal] Identity sync failed:", e);
        }
      };
      if (_NotificationService.oneSignalInitialized || OneSignal.initialized) {
        _NotificationService.oneSignalInitialized = true;
        await syncIdentity();
        _NotificationService.oneSignalInitInFlight = false;
        return;
      }
      try {
  ...
  markAsRead(id) {
    this.notifications.update((ns) => ns.map((n) => n.id === id ? __spreadProps(__spreadValues({}, n), { isRead: true }) : n));
    this.http.post(`${this.apiUrl}/${id}/mark-read`, {}).subscribe();
  }
  markAllAsRead() {
    const unreadIds = this.notifications().filter((n) => !n.isRead).map((n) => n.id);
    this.notifications.update((ns) => ns.map((n) => __spreadProps(__spreadValues({}, n), { isRead: true })));
  ...
  // ── Private helpers ────────────────────────────────────────────────────────
  loadNotifications() {
    this.http.get(`${this.apiUrl}`).subscribe({ next: (res) => this.notifications.set(res.data ?? []) });
  }
  startConnection() {
    ...
    this.hubConnection = new HubConnectionBuilder().withUrl(`${apiUrl}/hubs/notifications`, {
      skipNegotiation: false,
      transport: HttpTransportType.WebSockets,
      accessTokenFactory: () => this.authService.getToken() ?? ""
      // 🚀 بيقرأ التوكن الحي دايماً وقت إعادة الاتصال
    }).withAutomaticReconnect([0, 2e3, 5e3, 1e4]).build();
    this.hubConnection.on("ReceiveNotification", (notification) => {
      ...
      const emoji = emojiMap[notification.type] ?? "\u{1F514}";
      this.toastService.show(`${emoji} ${notification.title}`, notification.message, toastType, () => {
...
//# sourceMappingURL=chunk-CXPACYC7.js.map
