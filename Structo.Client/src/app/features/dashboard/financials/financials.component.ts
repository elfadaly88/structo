              ...
              <tbody class="text-sm divide-y divide-slate-800/60 text-slate-300">
                @for (request of pendingApprovals(); track request.id) {
                  <tr class="hover:bg-slate-950/20">
                    ...
                    <td class="py-4 text-slate-400 font-mono">{{ request.issuedAt | date:'yyyy-MM-dd HH:mm' }}</td>
                    <td class="py-4">
                      ...
                      @if (request.status === 'Issued') {
                        ...
                        <div class="flex items-center justify-center gap-2">
                          @if (request.receiptPhotoUrl) {
                            <a [href]="request.receiptPhotoUrl" target="_blank" class="text-indigo-400 hover:text-indigo-300 transition-colors inline-block cursor-pointer" title="View Receipt">
                          ...
                          @if (request.expenseDate) {
                            <span class="px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-slate-800 text-slate-400" title="Expense Date">
                              {{ request.expenseDate | date:'MMM d, yyyy' }}
                            </span>
                          }
                        </div>
                      } @else {
            ...
            <table class="w-full text-left rtl:text-right font-sans">
              ...
              <tbody class="text-sm divide-y divide-slate-800/60 text-slate-300">
                @for (transaction of transactions(); track transaction.id) {
                  <tr class="hover:bg-slate-950/20">
                    <td class="py-4 text-slate-400 font-mono">{{ transaction.transactionDate | date:'yyyy-MM-dd HH:mm' }}</td>
