...
export class TenantProfileComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.profileService.updateProfile(dto).subscribe({
      next: (res) => {
        ...
        if (res.success && res.data) {
          ...
          this.showToast('تم حفظ تعديلات بروفايل الشركة بنجاح! / Profile updated successfully.');
        } else {
          this.showToast(res.message || 'حدث خطأ أثناء الحفظ / Failed to update profile');
        }
      },
      error: (err) => {
        this.isSaving.set(false);
        this.showToast('تعذر حفظ البيانات، يرجى التكعّد من الاتصال بالشبكة.');
      }
    });
  }
  ...
  private initMap(): void {
    if (!this.profileMapContainer) return;
    ...
    if (!this.profileMap) {
      ...
      this.profileMarker = L.marker([lat, lng], { draggable: true }).addTo(this.profileMap);
